    ...
    return BlocConsumer<LikeCubit, LikeStates>(
      listener: (context, state) {
        if (state is LikeFailure) {
          ...
          widget.posts[widget.index].numberOfLikes =
              widget.posts[widget.index].numberOfLikes - 1;
          widget.posts[widget.index].isUserLike =
              !widget.posts[widget.index].isUserLike;
        } else if (state is BookMarkSuccess) {
          showToast(title: 'تم الحفظ بنجاح', color: AppColors.green);
        }
      },
          ...
          children: [
            Column(
              children: [
                GestureDetector(
                  ...
                  child: Icon(
                    widget.posts[widget.index].isUserLike == true
            ...
            Column(
              children: [
                InkWell(
                  onTap: () {
                    cubit.bookMark(postId: widget.posts[widget.index].postId);
                  },
                  child: Image.asset(
                    AppImages.bookmark,
                    width: 30,
